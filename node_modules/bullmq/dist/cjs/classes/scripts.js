...
class Scripts {
    constructor(queue) {
        this.queue = queue;
        this.finishedErrors = ({ code, jobId, parentKey, command, state, }) => {
            switch (code) {
    ...
    async remove(jobId, removeChildren) {
        ...
        const result = await client.removeJob(keys.concat([jobId, removeChildren ? 1 : 0]));
        if (result == enums_1.ErrorCode.JobBelongsToJobScheduler) {
            throw this.finishedErrors({
                code: enums_1.ErrorCode.JobBelongsToJobScheduler,
                jobId,
                command: 'remove',
            });
        }
